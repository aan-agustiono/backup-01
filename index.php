<?php
	$pth = isset($_REQUEST["dirs"])? ($_REQUEST["dirs"]): "jkt48" ;
...
echo '
	...
	<a class="nav-link dropdown-toggle" href="" id="dropdown01" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'.strtoupper($pth).'</a>
	<div class="dropdown-menu shadow-lg" aria-labelledby="dropdown01">
';

			$sl = opendir($dsl);
				while (($dirsel = readdir($sl))!==false){
						if(is_dir($dirsel)!== true  && $dirsel!==".." && $dirsel!=="." && $dirsel !== $pth ){
							 echo '<a class="dropdown-item" href="?dirs='.$dirsel.'">'.$dirsel.'</a>';
						}
				}			
...
?>					
    		...
    		<li class="nav-item"><a class="nav-link" href=""><img class="rounded-circle mr-2" src="assets/img/av.png" width="30"><span class="align-middle"><?php echo $_SERVER["REMOTE_ADDR"]; ?></span></a></li>
    		<li class="nav-item dropdown">
...
$dh  = opendir($dir);
    ...
    $match = preg_match("/.*\.(mp4|png|gif|jpeg|jpg|bmp)/", $file);
        if ($match) { 
		
		$lnkurl = $_SERVER["SERVER_NAME"].$url."/".$pth;		
			echo 	'
    			<div class="card card-pin">
    				<img class="card-img" src="http://'.$lnkurl.'/'.$file.'" alt="Card image">
    				<div class="overlay">
    					<h2 class="card-title title">'.$file.'</h2>
    					<div class="more">
    						<a href="post.php?dirs='.$pth.'&files='.$file.'" target="_blank">
